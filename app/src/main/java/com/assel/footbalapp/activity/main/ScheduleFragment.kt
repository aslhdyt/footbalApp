...
class ScheduleFragment: Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.recycler_layout, container, false).apply {
            ...
            when (eventType) {
                TYPE_LAST_EVENT -> viewModel.lastEvent.observe(this@ScheduleFragment, observer)
                TYPE_NEXT_EVENT -> viewModel.nextEvent.observe(this@ScheduleFragment, observer)
                TYPE_FAVOURITE -> viewModel.favouriteEvent.observe(this@ScheduleFragment, observer)
            }
        }
    }

    companion object {
        fun newInstance(eventType: Int): ScheduleFragment {
            val args = Bundle()
